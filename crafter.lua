...
-- "add_item" — экран сканирования сундука и добавления товара
local screen = "main"
local addItem = {
    scanned = false,
    label = nil, itemId = nil, damage = 0,
    name = "", craftAmount = 64, keepAmount = 64,
    activeField = nil,
    message = "",
}
...
-- Требуется: Adapter с Inventory Controller Upgrade, касающийся сундука.
-- =========================================================
local function scanChest()
    if not component.isAvailable("inventory_controller") then
        addItem.message = "Ошибка: нет Inventory Controller (нужен апгрейд в Adapter)"
        return
    end
...
local function saveAddItem()
    ...
    local newEntry = {
        ...
        name = finalName, craft_amount = craft, keep_amount = keep, enabled = true,
    }
...
local function loop()
    ...
    while true do
        ...
        elseif name == "key_down" then
            -- ВНИМАНИЕ: OpenComputers на разных версиях по-разному упаковывает
            -- (char, code) в событии key_down. Если ввод текста в поле
            -- "Название" печатает не то — поменяй местами a и b ниже.
            local a, b = ev[3], ev[4]
            if screen == "add_item" and addItem.activeField == "name" then
                ...
                if code == 28 then
                    addItem.activeField = nil
                elseif code == 14 then
                    addItem.name = addItem.name:sub(1, -2)
                elseif char and char >= 32 and char <= 126 then
                    addItem.name = addItem.name .. string.char(char)
                end
                redraw()
