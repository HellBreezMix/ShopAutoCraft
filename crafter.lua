...
local function pruneFinishedJobs()
    ...
    for key, j in pairs(activeJobs) do
        ...
        if s == "готов" then
            ...
            -- Считаем РЕАЛЬНЫЙ прирост остатка (produced), а не номинальную цель
            -- заказа (amount) — если игрок что-то забрал/подложил во время
            -- крафта, produced честно отражает фактическую разницу.
            -- ВАЖНО: 0 в Lua — истинное значение, поэтому "j.produced or j.amount"
            -- не переключался бы на amount, если produced ещё не успел
            -- замериться (AE2 сказал "готово" раньше нашей проверки склада).
            local qty = j.produced
            if not qty or qty <= 0 then qty = j.amount or 0 end
            dailyItemCounts[j.name] = (dailyItemCounts[j.name] or 0) + qty
...
local function updateProgress(stocksFromTick)
    ...
    for key, j in pairs(activeJobs) do
        ...
        if (j.produced or 0) >= j.amount then
            ...
            if not qty or qty <= 0 then qty = j.amount or 0 end
            dailyItemCounts[j.name] = (dailyItemCounts[j.name] or 0) + qty
            logJob("ЗАВЕРШЁН", j.name .. " x" .. j.amount .. " (по остатку)")
            pcall(function() j.job.cancel() end)
            activeJobs[key] = nil
        end
    end
...
-- Номер из последнего "!товары" -> {id, damage}, чтобы "!порог 3 500" знал,
-- какой именно товар имеется в виду. Сбрасывается/перезаписывается каждый
-- раз при новом вызове "!товары".
local lastShownItems = {}

-- Показывает пронумерованный список товаров с РЕАЛЬНЫМ остатком из ME-сети
-- (не только настроенный порог). Режем на порции по 25 строк — иначе легко
-- упереться в лимит Discord на длину сообщения (2000 символов).
local function cmdListItems()
    local items = loadCrafterItems()
    if not items or #items == 0 then
        discord.send("Список товаров пуст.")
        return
    end
    ...
    for i, it in ipairs(items) do
        ...
        lastShownItems[i] = { id = it.id, damage = dmg }
        local key = (it.id or "") .. "|" .. dmg
        local stockStr = gotStocks and tostring(stocks[key] or 0) or "?"
        local mark = (it.enabled == false) and " [выкл]" or ""
        table.insert(lines, string.format("%d. %s — остаток:%s / порог:%s / крафт:%s%s",
            i, it.name or it.id or "?", stockStr,
            tostring(it.keep_amount), tostring(it.craft_amount), mark))
    end

    local CHUNK = 25
    for start = 1, #lines, CHUNK do
        local part = {}
        for i = start, math.min(start + CHUNK - 1, #lines) do table.insert(part, lines[i]) end
        discord.send("📦 **Товары**" .. (not gotStocks and " (нет связи с ME — остаток не показан)" or "") ..
            "\n```\n" .. table.concat(part, "\n") .. "\n```")
    end
end

-- Меняет keep_amount ("порог") или craft_amount ("крафт") у товара с номером
-- idx из последнего показанного "!товары". Читает файл заново перед записью,
-- чтобы не затереть изменения, сделанные параллельно через GUI.
local function cmdSetField(field, idxStr, valueStr, author)
    local idx = tonumber(idxStr)
    local value = tonumber(valueStr)
    if not idx or not value or value < 0 then
        discord.send("⚠ Формат: `!порог <номер> <значение>` или `!крафт <номер> <значение>`. Сначала вызови `!товары`.")
        return
    end
    local ref = lastShownItems[idx]
    if not ref then
        discord.send("⚠ Номер " .. idx .. " не найден — сначала вызови `!товары`, чтобы обновить список.")
        return
    end
    ...
    local found = nil
    for _, it in ipairs(items) do
        if it.id == ref.id and math.floor(tonumber(it.damage) or 0) == ref.damage then
            found = it
            break
        end
    end
    if not found then
        discord.send("⚠ Товар не найден в текущем файле (возможно, был удалён) — обнови `!товары`.")
        return
    end

    if field == "keep_amount" then found.keep_amount = value
    else found.craft_amount = value end

    local f = io.open(path, "w")
    if not f then
        discord.send("⚠ Не удалось сохранить файл")
        return
    end
    f:write(json.encode({ items = items }))
    f:close()
    cachedItems = nil  -- следующий тик подхватит изменения

    local fieldLabel = (field == "keep_amount") and "порог" or "размер партии крафта"
    log("КОМАНДА", author .. ": " .. fieldLabel .. "[" .. (found.name or found.id) .. "]=" .. value)
    discord.send("✅ " .. (found.name or found.id) .. " — " .. fieldLabel .. " изменён на " .. value .. " (" .. author .. ")")
end

-- Включает/выключает товар с номером idx из последнего "!товары".
local function cmdSetEnabled(idxStr, value, author)
    local idx = tonumber(idxStr)
    if not idx then return end
    local ref = lastShownItems[idx]
    if not ref then
        discord.send("⚠ Номер " .. idx .. " не найден — сначала вызови `!товары`.")
        return
    end
    ...
    if not found then
        discord.send("⚠ Товар не найден в текущем файле — обнови `!товары`.")
        return
    end
    ...
    log("КОМАНДА", author .. ": " .. (value and "включен" or "выключен") .. " [" .. (found.name or found.id) .. "]")
    discord.send((value and "✅ " or "🚫 ") .. (found.name or found.id) ..
        " — " .. (value and "включён" or "выключен") .. " (" .. author .. ")")
end

-- Ищет по всей ME-сети предметы, где в названии встречается подстрока query
-- (регистронезависимо). Не зависит от crafter_data.json — находит ЛЮБОЙ
-- предмет, который хоть раз проходил через сеть, вместе с его id/damage —
-- удобно, чтобы узнать id не заходя в игру физически.
local function cmdFindItems(query, author)
    query = (query or ""):lower()
    if query == "" then
        discord.send("⚠ Формат: `!найти <часть названия>`, например `!найти алмаз`")
        return
    end
    ...
    for addr in component.list("me_interface") do
        local proxy = component.proxy(addr)
        local ok, items = pcall(function() return proxy.getItemsInNetwork() end)
        if ok and items then found = items; break end
    end
    if not found then
        discord.send("⚠ Нет связи с ME-сетью")
        return
    end

    local matches = {}
    for _, it in ipairs(found) do
        local label = it.label or it.name or ""
        if label:lower():find(query, 1, true) then
            table.insert(matches, {
                label = label, id = it.name, damage = math.floor(it.damage or 0), size = it.size or 0,
            })
        end
    end
    ...
    if #matches == 0 then
        discord.send("Ничего не найдено по запросу \"" .. query .. "\"")
        return
    end
    ...
    for i = 1, math.min(MAX, #matches) do
        local m = matches[i]
        table.insert(lines, m.label .. " — id=" .. m.id .. " damage=" .. m.damage .. " остаток=" .. m.size)
    end
    local body = table.concat(lines, "\n")
    if #matches > MAX then
        body = body .. "\n... и ещё " .. (#matches - MAX) .. " совпадений (уточни запрос)"
    end
    discord.send("🔍 **Найдено (\"" .. query .. "\")**\n```\n" .. body .. "\n```")
end

-- Добавляет/обновляет товар в crafter_data.json прямо командой из Discord.
-- Формат: !добавить <id> <damage> <крафт> <порог> <название...>
-- (название — последним и может содержать пробелы, остаток строки целиком)
local function cmdAddItem(rawText, author)
    local id, dmgStr, craftStr, keepStr, name =
        rawText:match("^!добавить%s+(%S+)%s+(%d+)%s+(%d+)%s+(%d+)%s+(.+)$")
    if not id then
        discord.send("⚠ Формат: `!добавить <id> <damage> <крафт> <порог> <название>`\n" ..
            "Например: `!добавить minecraft:diamond 0 64 100 Алмаз`\n" ..
            "Не знаешь id — сначала `!найти <часть названия>`")
        return
    end
    local dmg = tonumber(dmgStr)
    local craft = tonumber(craftStr)
    local keep = tonumber(keepStr)
    name = name:gsub("%s+$", "")
    ...
    local newEntry = { id = id, damage = dmg, name = name, craft_amount = craft, keep_amount = keep, enabled = true }
    local replaced = false
    for i, it in ipairs(items) do
        if it.id == id and math.floor(tonumber(it.damage) or 0) == dmg then
            items[i] = newEntry
            replaced = true
            break
        end
    ...
    log("КОМАНДА", author .. ": " .. (replaced and "обновлён" or "добавлен") .. " товар " .. name)
    discord.send("✅ " .. (replaced and "Обновлено: " or "Добавлено: ") .. name ..
        " (id=" .. id .. ", damage=" .. dmg .. ", крафт=" .. craft .. ", порог=" .. keep .. ") — " .. author)
end
...
local function handleCommand(text, author)
    ...
    elseif cmd == "!товары" or cmd == "!items" then
        log("КОМАНДА", author .. ": товары")
        cmdListItems()
    elseif cmd:match("^!найти%s+") or cmd:match("^!find%s+") then
        local query = raw:match("^!найти%s+(.+)$") or raw:match("^!find%s+(.+)$")
        log("КОМАНДА", author .. ": найти " .. tostring(query))
        cmdFindItems(query, author)
    elseif cmd:match("^!добавить%s+") then
        cmdAddItem(raw, author)
    else
        local n = cmd:match("^!лимит%s+(%d+)$") or cmd:match("^!limit%s+(%d+)$")
        local pIdx, pVal = cmd:match("^!порог%s+(%d+)%s+(%d+)$")
        local cIdx, cVal = cmd:match("^!крафт%s+(%d+)%s+(%d+)$")
        local eIdx = cmd:match("^!вкл%s+(%d+)$")
        local dIdx = cmd:match("^!выкл%s+(%d+)$")
        ...
        elseif pIdx then
            cmdSetField("keep_amount", pIdx, pVal, author)
        elseif cIdx then
            cmdSetField("craft_amount", cIdx, cVal, author)
        elseif eIdx then
            cmdSetEnabled(eIdx, true, author)
        elseif dIdx then
            cmdSetEnabled(dIdx, false, author)
        elseif cmd == "!help" or cmd == "!помощь" then
            discord.send(
                "📖 **Доступные команды:**\n" ..
                "`!статус` — текущее состояние крафтера\n" ..
                "`!товары` — список товаров с реальными остатками\n" ..
                "`!найти <часть названия>` — искать предмет по ME-сети (узнать id без похода в игру)\n" ..
...
-- =========================================================
-- ВЕБ-ПАНЕЛЬ
-- Раз в WEB_STATUS_INTERVAL секунд шлём снимок текущего состояния на Worker —
-- он хранит его в KV и отдаёт как HTML-страницу при заходе в браузере.
-- =========================================================
local WEB_STATUS_INTERVAL = 60
local lastWebStatusAt = -WEB_STATUS_INTERVAL

local function pushWebStatus()
    local now = computer.uptime()
    if (now - lastWebStatusAt) < WEB_STATUS_INTERVAL then return end
    lastWebStatusAt = now
    if not component.isAvailable("internet") then return end

    local items = loadCrafterItems() or {}
    local stocks, gotStocks = getStocks()
    ...
    for _, it in ipairs(items) do
        ...
        table.insert(itemsOut, {
            name = it.name or it.id or "?",
            stock = gotStocks and (stocks[key] or 0) or 0,
            keep = tonumber(it.keep_amount) or 0,
            craft = tonumber(it.craft_amount) or 0,
    ...
    for _, j in pairs(activeJobs) do
        table.insert(jobsOut, {
            name = j.name, produced = j.produced or 0, amount = j.amount,
            status = jobStatus(j),
        })
    end
    ...
    pcall(commands.pushStatus, payload)
end
...
local function loop()
    ...
    while true do
        ...
        pushWebStatus()  -- rate-limited внутри (раз в WEB_STATUS_INTERVAL сек)
        ...
        elseif not name then
            -- Перерисовываем по таймауту только на главном экране (там "живой"
            -- обратный отсчёт до тика). На экранах добавления/списка товаров
            -- ничего не меняется само по себе — лишняя перерисовка каждую
            -- секунду там только мигает кнопками без всякой пользы.
            if screen == "main" then redraw() end
        end
    end
end
