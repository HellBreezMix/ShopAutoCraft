...
local function pruneFinishedJobs()
    ...
    for key, j in pairs(activeJobs) do
        ...
        if s == "готов" then
            ...
            dailyItemCounts[j.name] = (dailyItemCounts[j.name] or 0) + 1
...
local function sendDailySummary()
    ...
    for i = 1, math.min(3, #sorted) do
        table.insert(topLines, sorted[i].name .. " x" .. sorted[i].cnt)
    end
    local topStr = (#topLines > 0) and table.concat(topLines, ", ") or "—"

    discord.send(string.format(
        "📅 **Суточная сводка**\nСкрафчено: %d\nПровалено: %d\nТоп товаров: %s\nАктивно сейчас: %d/%d | ME=%s",
        dailyCompleted, dailyFailed, topStr, countActive(), maxConcurrent,
