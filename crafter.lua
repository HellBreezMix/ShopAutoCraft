...
local function pruneFinishedJobs()
    ...
    for key, j in pairs(activeJobs) do
        ...
        if s == "готов" then
            ...
            -- Считаем РЕАЛЬНЫЙ прирост стока (produced), а не номинальную цель
...
local function cmdListItems()
    ...
    for i, it in ipairs(items) do
        ...
        table.insert(lines, string.format("%d. %s — сток:%s / порог:%s / крафт:%s%s",
    ...
    for start = 1, #lines, CHUNK do
        ...
        discord.send("📦 **Товары**" .. (not gotStocks and " (нет связи с ME — сток не показан)" or "") ..
            "\n```\n" .. table.concat(part, "\n") .. "\n```")
    end
end
...
local function cmdFindItems(query, author)
    ...
    for i = 1, math.min(MAX, #matches) do
        local m = matches[i]
        table.insert(lines, m.label .. " — id=" .. m.id .. " damage=" .. m.damage .. " сток=" .. m.size)
    end
