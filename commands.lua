...
-- Используется POST (не GET) — Cloudflare более строго проверяет GET-запросы
-- от Java-клиента OC и отдаёт 403, а POST на этот же домен уже проверенно
-- работает (через него шлются уведомления в discord.lua).
...
function commands.poll()
    ...
    local pollUrl = base .. "/poll"
    local body = json.encode({ after = lastId })
    local headers = { ["Content-Type"] = "application/json" }

    local handle
    local ok, result = pcall(function()
        handle = internet.request(pollUrl, body, headers, "POST")
        local parts = {}
