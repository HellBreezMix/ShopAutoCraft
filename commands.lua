...
function commands.poll()
    ...
    local headers = {
        ...
        -- ВАЖНО: этого заголовка не хватало — без него Cloudflare отдавал 403,
        -- как раньше было при прямых запросах к Discord (см. discord.lua).
        ["User-Agent"] = "OpenComputers-Crafter/1.0 (+https://github.com)",
    }
...
-- commands.pushStatus(data) — отправляет снимок статуса для веб-панели.
-- "Best effort": любая ошибка гасится молча, страница просто не обновится
-- до следующей успешной попытки.
function commands.pushStatus(data)
    local base = config.discord_webhook_url
    if not base or base == "" then return false end
    local body = json.encode({ status = data })
    local headers = {
        ["Content-Type"] = "application/json",
        ["User-Agent"] = "OpenComputers-Crafter/1.0 (+https://github.com)",
    }
