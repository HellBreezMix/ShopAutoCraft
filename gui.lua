...
function gui.drawAddItem(state)
    ...
    txt(2, 12, "Статус:", gui.COLORS.label, gui.COLORS.bg)
    local enabled = (state.enabled ~= false)
    gui.btn("toggle_enabled", 20, 12, 16, 1,
        enabled and "[ВКЛЮЧЁН]" or "[ВЫКЛЮЧЕН]",
        enabled and gui.COLORS.good or gui.COLORS.bad)

    -- ===== Кнопка сохранения =====
    gui.btn("save", 2, 14, 20, 1, "[СОХРАНИТЬ]", gui.COLORS.good)

    -- ===== Сообщение/статус =====
    rect(1, 16, W, 1, gui.COLORS.bg)
    if state.message and state.message ~= "" then
        ...
        txt(2, 16, state.message, msgCol, gui.COLORS.bg)
    end

    txt(2, 18, "Подсказка: тапни по полю \"Название\", чтобы переименовать, " ..
